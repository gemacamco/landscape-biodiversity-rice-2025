...
# load dataset
setwd("C:/R_files/git_open/meta/data")                      # REVISAR
...
build_model <- function(data, mods = NULL, Change.Opt = FALSE) {
...
}  # ends buil_model function
################################################################################
# META-ANALYSIS LABELS - select which analysis you want to run 

# 1.1. Landscape complexity effect on biodiversity  -> "general"                # dim_Hemiptera da problema con el optimizer          
...
# 3.1. Landscape dimensions effect on amphibians  -> "dim_Anura" (only composition)
# 3.2. Landscape dimensions effect on birds       -> "dim_birds" (only composition)
# 3.3. Landscape dimensions effect on Araneae     -> "dim_Araneae"
# 3.4. Landscape dimensions effect on Coleoptera  -> "dim_Coleoptera"
# 3.5. Landscape dimensions effect on Hemiptera   -> "dim_Hemiptera"
# 3.6. Landscape dimensions effect on Lepidoptera -> "dim_Lepidoptera"
# 3.7. Landscape dimensions effect on Hymenoptera -> "dim_Hymenoptera"
# 3.8. Landscape dimensions effect on Odonata     -> "dim_Odonata"
...
switch(LB,
       ...
       "dim_Hemiptera" = {
         ...
         mods <- ~g_indicator
         mod <- build_model(data_model, mods = mods, Change.Opt = TRUE)
       },
       "dim_Lepidoptera" = {
...
# 7. Outliers analysis
  # 7.1 Fit the model
 # mod <- tryCatch({build_model(data_model, mods = mods, Change.Opt = FALSE)},
 #                  warning = function(cond){cat(paste0("Model Warning: ", cond))},
 #                  error   = function(cond){cat(paste0("Model Error: ", cond))
 #                    return(NULL)})
  
  # Check Optimizer
  # if(is.null(mod)){mod <- tryCatch({build_model(data_model, mods = mods, Change.Opt = TRUE)},
  #                                  warning = function(cond){cat(paste0("Model Warning: ", cond))},
  #                                  error   = function(cond){cat(paste0("Model Error: ", cond))
  #                                    return(NULL)})}
  
  # 7.2 Identify outliers using Cook's distance
  x <- cooks.distance(mod, progbar = TRUE, parallel = "snow", ncpus = 12)  # Calculate Cook's distance
  threshold <- 4 / length(x)  # Define outlier threshold
  outliers <- data_model$id_r[which(x > threshold)]  # Identify outliers
  
    # Remove outliers
    data_out <- data_model %>% filter(!id_r %in% outliers)
    mod_out <- tryCatch({build_model(data = data_out, mods = mods, Change.Opt = FALSE)},
                        warning = function(cond){cat(paste0("Model Warning: ", cond))},
                        error   = function(cond){cat(paste0("Model Error: ", cond))
                          return(NULL)})
    summary(mod_out)
